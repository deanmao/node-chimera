...
try {
  if (fs.realpathSync(__dirname + '/../build')) {
    console.log('using build version');
    webkit = require(__dirname + '/../build/Release/chimera.node');
  }
} catch (e) {
  console.log('using release version');
  webkit = require(__dirname + '/../native/'+process.platform+'/chimera.node');
}
if (webkit === null) {
  throw new Error('Cannot find appropriate binary library for webkit');
}
